...
To record a demo video for evaluation, follow this recommended flow:

1. **Terminal — Run the pipeline:**
   ```bash
   python run.py --policy compliance_policy.pdf --data data_run/
   ```
   Show the policy being parsed, violations being detected, and the summary table.

2. **Dashboard — Live Feed Monitor:**
   - Launch `streamlit run src/dashboard.py`
   - Upload a video clip from `data_run/`
   - Click "Analyze" — show the detection results and alert banner

3. **Dashboard — Alert Timeline:**
   - Switch to Alert Timeline view
   - Show the auto-refreshing event stream

4. **Dashboard — Historical Log:**
   - Switch to Historical Log view
   - Demonstrate date range, severity, and behavior class filters
   - Click "Export to CSV" and show the downloaded file

5. **Code Walkthrough (optional):**
   - Briefly show `policy_rules.json` (output of policy parsing)
   - Show the `violations.db` contents via the dashboard table
